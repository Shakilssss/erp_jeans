...
function sal_summary_with_ot_eot_date_between(){
	...
	var grid_status = document.getElementById('grid_status').value;
	var first_date = document.getElementById('from_date').value;
	var second_date = document.getElementById('to_date').value;
	...
	var sal_year_month = report_year_sal+"-"+report_month_sal+"-"+"01";
	hostname = window.location.hostname;
	url =  "http://"+hostname+"/erp_jeans/index.php/salary_report_con/sal_summary_with_ot_eot_date_between/"+sal_year_month+"/"+grid_status+"/"+first_date+"/"+second_date;
	sal_sheet = window.open(url,'sal_sheet',"menubar=0,resizable=1,scrollbars=1,width=1600,height=800");
	sal_sheet.moveTo(0,0);
}
...
function grid_job_card()
{
	...
	ajaxRequest.onreadystatechange = function(){
		if(ajaxRequest.readyState == 4){
			var resp = ajaxRequest.responseText;
			$(".clearfix").dialog("close");
			ot_report = window.open('', '_blank', 'menubar=1,resizable=1,scrollbars=1,width=1600,height=800');
			ot_report.document.write(resp);
			ot_report.stop();			
		}
	}
}
...
function leave_register(){
 var ajaxRequest;  // The variable that makes Ajax possible!
	...
	var second_date = document.getElementById('seconddate').value;
	$grid  = $("#list1");
	var id_array = $grid.getGridParam('selarrrow');
	var selected_id_list = new Array();
	var spl = (id_array.join('xxx'));
	...
	var queryString="spl="+spl+"&first_date"+first_date+"&second_date="+second_date;
	url =  hostname+"index.php/grid_con/leave_register/";
	$(".clearfix").dialog("open");
	ajaxRequest.open("POST", url, true);
	ajaxRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded;charset=utf-8");
	ajaxRequest.send(queryString);
