...
//============================= Maintainance Functions=============================================	
	...
	/*function rename_emp_id()
	{
		...
		for($i=0; $i<=5; $i++)
		{
			//echo "$i => $data[$i] ====>>>> $i => $data1[$i]<br>";
			
			$this->db->select('emp_id');
			$this->db->where('emp_id', $data[$i]);
			$query = $this->db->get('pr_emp_com_info');
			
			if($query->num_rows() > 0)
			{
				echo "$i. This ('$data[$i]') ID is exist and Renaming to $data1[$i].....<br>"; 
				
				$this->db->select('emp_id');
				$this->db->where('emp_id', $data1[$i]);
				...
				$row_num2 = $query2->num_rows();
				
				$this->db->select('emp_id');
				$this->db->where('emp_id', $data1[$i]);
				$query3 = $this->db->get('pr_emp_per_info');
				$row_num3 = $query3->num_rows();
				
				$this->db->select('emp_id');
				$this->db->where('emp_id', $data1[$i]);
				$query4 = $this->db->get('pr_emp_skill');
				$row_num4 = $query4->num_rows();
				
				$this->db->select('emp_id');
				$this->db->where('emp_id', $data1[$i]);
				$query5 = $this->db->get('pr_id_proxi');
				$row_num5 = $query5->num_rows();
				
				$this->db->select('emp_id');
				$this->db->where('emp_id', $data1[$i]);
				$query6 = $this->db->get('pr_emp_edu');
				$row_num6 = $query6->num_rows();
			
			
				
				if($row_num1 == 0 and $row_num2 == 0 and $row_num3 == 0 and $row_num4 == 0 and $row_num5 == 0 and $row_num6 == 0)
				{
					
					$updated_data = array('emp_id' => $data1[$i]);
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_emp_add', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_emp_com_info', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_emp_per_info', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_emp_skill', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_id_proxi', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$this->db->where('emp_id', $data[$i]);
					$this->db->update('pr_emp_edu', $updated_data);
					$last_query = $this->db->last_query();
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $last_query<br>"; 
					
					$old_table = "temp_$data[$i]";
					$new_table = "temp_$data1[$i]";
					...
					echo "> $last_query<br>"; 
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> Successfully Renamed This ('$data[$i]') ID to $data1[$i].<br>"; 
				}
				else
				{
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> Can't Rename This ('$data[$i]') ID to $data1[$i].<br>"; 
					echo "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;";
					echo "> $row_num1, $row_num2, $row_num3, $row_num4, $row_num5, $row_num6<br>";
				}
			}
			else
			{
				echo "$i. Sorry!,  This ('$data[$i]') ID does not exist.<br>"; 
			}
		}
