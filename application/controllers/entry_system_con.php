...
class Entry_system_con extends CI_Controller {
	...
	function manual_entry_Delete()
	{
		...
		// new code write 04/10/21
		$this->load->model('file_process_model');
		$data = $this->file_process_model->delete_manual_entry($grid_firstdate, $grid_seconddate, $grid_emp_id);
	...
	function save_holiday()
	{
		...
		$data = $this->grid_model->save_holiday($grid_firstdate,$grid_seconddate,$holiday_description,$grid_emp_id);
		echo $data;
	}
	//-------------------------------------------------------------------------------------------------------
	// Resign Entry
	...
	//-------------------------------------------------------------------------------------------------------
	// Maternity Entry New by Tarek on 22-6-16
	//-------------------------------------------------------------------------------------------------------
	function maternity_entry()
	{
		$crud = new grocery_CRUD();

		$crud->set_table('pr_emp_meternity_history');
		$crud->set_subject('Maternity Employee');
		$crud->required_fields('emp_id','line_no','start_date','end_date');
		$crud->display_as( 'emp_id' , 'Employee ID' );
		//$crud->set_rules('emp_id','Employee ID','required|is_unique[pr_emp_meternity_history.emp_id]|callback_username_check');
		$crud->set_relation('emp_id', 'pr_emp_com_info', 'emp_id');
		$crud->set_relation('line_no', 'pr_line_num', 'line_name');
	...
	//-------------------------------------------------------------------------------------------------------
	function weekend_delete()
	{
		$crud = new grocery_CRUD();

		$crud->set_table('pr_work_off');
		$crud->set_subject('Weekend');
	...
	//-------------------------------------------------------------------------------------------------------
	// Manual lunch leave Entry ( full day to half day present)
	//-------------------------------------------------------------------------------------------------------
	function manual_lunch_leave_entry()
	{
		...
		$this->load->model('file_process_model');
		$data = $this->file_process_model->manual_lunch_leave_entry($grid_firstdate, $grid_seconddate, $manual_time, $grid_emp_id, $leave_type);
		echo $data;
	}
	//-------------------------------------------------------------------------------------------------------
	// Lunch leave Delete manually (half day to full day present)
	//-------------------------------------------------------------------------------------------------------
	function manual_lunch_leave_entry_delete()
	{
		...
		$this->load->model('file_process_model');
		$data = $this->file_process_model->manual_lunch_leave_entry_delete($grid_firstdate, $grid_seconddate, $grid_emp_id);
