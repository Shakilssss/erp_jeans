...
<div style=" margin:0 auto;  overflow:hidden; font-family: 'Times New Roman', Times, serif; width:100%; ">
	<div  style="font-size:13px; font-weight:bold; text-align:center; width:100%;">
		Monthly Salary Summary with OT of 
		<?php 
			$date = $salary_month;
	...
	<br/>
<table class="sal" border="1" cellspacing="0" cellpadding="0" style="font-size:12px; width: 800px; margin: 0 auto;">
 	...
 	<?php
		...
		foreach ($values as $row){
			...
			if ($row["line_manPower"][0]['t_emp'] !=0) {
				...
				echo "<td align='right'>";
				echo number_format($row["line_manPower"][0]['t_emp'] * 10 );
				echo "</td>";

				echo "<td align='right'>";
				echo number_format($row["line_manPower"][0]['t_deduct_amt']);
				echo "</td>";
				
				echo "<td align='right'>";
				$s_total = $row["line_manPower"][0]['t_wages']+$row["line_manPower"][0]['t_bonus']+$row["line_manPower"][0]['t_ot_amt'] - $row["line_manPower"][0]['t_deduct_amt'];
				echo number_format($s_total - ($row["line_manPower"][0]['t_emp'] * 10));
				echo "</td>";
				...
				$g_deduct_amt = $g_deduct_amt+$row["line_manPower"][0]['t_deduct_amt'];
		...
		echo "<td align='right'>";
		echo number_format($g_total_emp * 10);
		echo "</td>";

		echo "<td align='right'>";
		echo number_format($g_deduct_amt);
		echo "</td>";
		
		echo "<td align='right'>";
		echo number_format($g_total_amt - ($g_total_emp * 10));
		echo "</td>";
