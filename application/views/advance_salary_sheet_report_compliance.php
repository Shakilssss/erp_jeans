...
<title>arafAdvance Salary Sheet</title>
...
?>
  ...
  <tr height="20px">
    ...
    <td width="35" height="20px"><div align="center"><strong>20 Days Salary Payable Amount</strong></div></td>
...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		echo $payable_amount = round(($gross_sal / $last_day) * 20);
		echo "</td>";
		...
		/*$query = $this->db->where('emp_id', $emp_id)->where('loan_date', '2017-08-01')->where('loan_status', 1)->get('pr_advance_loan');
		$data = array(
					'emp_id' 		=> $emp_id,
					'loan_amt'		=> $net_pay,
					'pay_amt'		=> $net_pay,
					'loan_date'		=> '2017-08-01',
					'loan_status'	=> 1
					);
			if($query->num_rows() > 0 ){
