...
<?php
	...
	for($p=0; $p<=$per_page_row;$p++)
	{
		...
		echo "<td>";
		$bouns_rule = $this->salary_process_model->get_festival_bonus_rule($service_month, $effective_date);
		if($bouns_rule)
		...
		echo "<td>";
		print_r ($value[$k]->festival_bonus);
		$total_festival_bonus = $total_festival_bonus + $value[$k]->festival_bonus;
		$net_pay = $net_pay + $value[$k]->festival_bonus;
		echo "</td>";
