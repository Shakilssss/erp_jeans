...
class Leave_model extends CI_Model{
	...
	{	
		...
		else
		{
			...
			if($leave_type == "el")
			{
				$this->db->select('current_earn_balance');
				$this->db->where("emp_id", $empid_leave);
				$query = $this->db->get('pr_leave_earn');
				$rows = $query->row();
				$leave_balance = $rows->current_earn_balance;
				//echo $leave_balance;
			}
			else
			{
	...
	function leave_insert($emp_status,$leave_type,$empid_leave,$sStartDate,$sEndDate)
	{
		...
		if($leave_type == "el")
		{
			...
			$data = array(
               'current_earn_balance' =>$earn_balance,
	...
	function leave_transaction_db()
	{
	 ...
	 //leave balance test
		...
		if($query->num_rows() > 0){
			$rows = $query->row();
			$earn_leave_balance = $rows->current_earn_balance;
		}else{
